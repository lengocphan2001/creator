@php use App\Helpers\CreatorHelper; @endphp
...
@section('admin_main')
    ...
    <div id="layout-wrapper">
        ...
        @include('creator.includes.header')
            ...
            <div data-simplebar class="h-100">
                ...
                <div id="sidebar-menu">
                    ...
                    <ul class="metismenu list-unstyled" id="side-menu">
                        @foreach(CreatorHelper::getCreatorSidebar() as $menu)
                            <li>
                                
                                <a href="{{ (!isset($menu['items']) && !empty($menu['route'])) ? route("creator.{$menu['route']}") : 'javascript:void(0);' }}">
                                    <i class="{{$menu['icon']}}"></i>
                                ...
                                @if(isset($menu['items']))
                                    <ul class="sub-menu" aria-expanded="false">
                                        @foreach($menu['items'] as $subMenu)
                                            <li>
                                                <a href="{{ !empty($subMenu['route']) ? route("creator.{$subMenu['route']}") : 'javascript:void(0);' }}">{{ $subMenu['label'] }}</a>
                                            </li>
                                        @endforeach
                                    </ul>
                                @endif
                    ...
                    <div class="row">
                        <div class="col-sm-6">
                            Copyright {{ date('Y') }} © to <a
                                href="{{ route('creator.dashboard') }}"><b>{{ config('app.name') }}</b></a>.
                        </div>
